...
def sparse_continents
  # Find every country that belongs to a 1)continent where each country's
  # population is less than 25,000,000. Show name, continent and
  # population.

  # Show name, continent and population of countries that belong to a continent...
  ...
  FROM countries
  WHERE population < 25000000 AND continent IN
    (SELECT continent
    FROM countries
    WHERE <= ALL (population < 25000000);)  
  SQL
end
